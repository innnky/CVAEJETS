...
class AudioTextProcessor(object):
    def __init__(self, preprocess_config, preprocessing=False):
        ...
        self.text_cleaners = preprocess_config["preprocessing"]["text"]["text_cleaners"]
...
class StatParser(AudioTextProcessor):
    def __init__(self, preprocess_config, preprocessing=True):
        ...
        self.speakers = preprocess_config["preprocessing"]["speakers"]
        self.energy_scaler = StandardScaler()
        with open(self.raw_path, 'r', encoding='utf8') as f:
            self.lines = f.read().split('\n')
            self.lines = [l for l in self.lines if len(l) > 0]
            self.tmp = []
            self.energies = []
    ...
    def __call__(self):
        ...
        with open(os.path.join(self.out_dir, 'speakers.json'), 'w', encoding='utf8') as f:
            speakers_dict = {speaker:idx for idx, speaker in enumerate(self.speakers)}
            f.write(json.dumps(speakers_dict))
